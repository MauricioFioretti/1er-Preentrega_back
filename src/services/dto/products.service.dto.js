export function productsServiceDTO(productos, limit, sort, query) {
    ...
    productos.prevLink = productos.prevPage ? `http://localhost:8080/products?page=${productos.prevPage}&limit=${limit}&query=${query ? `&query=${query}` : ''}&sort=${sort}` : null

    productos.nextLink = productos.nextPage ? `http://localhost:8080/products?page=${productos.nextPage}&limit=${limit}&query=${query ? `&query=${query}` : ''}&sort=${sort}` : null
